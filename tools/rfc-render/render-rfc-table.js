...
async function render() {
  ...
  for (const issue of result) {
    ...
    if (doc) {
      ...
      link = `https://github.com/gocreateio/honey-rfcs/pull/${pr_number}`;
    } else {
      link = `https://github.com/gocreateio/honey-rfcs/issues/${issue.number}`;
    }
  ...
  for (const issues of Object.values(issueByStatus)) {
    for (const row of issues.sort(byNumber)) {
      const cols = [
        `[${row.number}](https://github.com/gocreateio/honey-rfcs/issues/${row.number})`,
        `[${row.title}](${row.link})`,
